...
class DB_Opt3BitBFS2_Solver {
public:
    DB_Opt3BitBFS2_Solver(
    ...
    uint64_t Expand(int segment) {
        ...
        for (int op = 0; op < SOpts.OperatorsCount; op++) {
            while (true) {
                ...
                if (XSegUsedOpBits[op].DataSize() == 0) {
                    XSegUsedOpBits[op].Resize(SOpts.SegmentSize);
                    XSegUsedOps.push_back(op);
                }
                for (size_t i = 0; i < vect.Size(); i++) {
                    uint32_t idx = vect[i];
                    NextArray.Set(idx);
                    XSegUsedOpBits[op].Set(idx);
                }
            }
        }
        ...
        auto fnScanApply = [&](uint64_t index) {
            count++;
            int opBits = 0;
        ...
        if (SOpts.HasOddLengthCycles) {
            NextArray.ScanBitsAndClearWithExcl(fnScanApply, CurArray);
            CurArray.Clear();
        }
        else {
            NextArray.ScanBitsAndClear(fnScanApply);
        }
        Expander.Finish(fnExpandCrossSegment);
        FrontierWriter.Flush();
        for (int op : XSegUsedOps) {
            XSegUsedOpBits[op].Clear();
        }
...
private:
    ...
    std::vector<BitArray> XSegUsedOpBits;
    std::vector<int> XSegUsedOps;
...
std::vector<uint64_t> DiskBasedOptThreeBitBFS2(Puzzle& puzzle, std::string initialState, PuzzleOptions opts) {
    Timer timer;
    std::cerr << "DiskBasedOptThreeBitBFS2" << std::endl;
    ...
    std::vector<std::unique_ptr<DB_Opt3BitBFS2_Solver>> solvers;
    for (int i = 0; i < opts.threads; i++) {
        solvers.emplace_back(std::make_unique<DB_Opt3BitBFS2_Solver>(
            sopts,
            oldFrontierStore,
            curFrontierStore,
            newFrontierStore,
